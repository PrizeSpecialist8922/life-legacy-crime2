...
c.money = 200000;
if (c.crime!.trial) c = resolveTrial(c, "plea").character;
...
r = resolveTrial(c, "inform");
check(r.ok, "informing works");
c = r.character;
check(
...
const runLife = (careful: boolean) => {
  ...
  for (let y = 0; y < 25 && ch.alive; y++) {
    ...
    if (ch.crime?.trial) ch = resolveTrial(ch, careful ? "fight" : "plea").character;
...
try {
  for (let i = 0; i < 25; i++) {
    ...
    if (rc.crime?.trial) rc = resolveTrial(rc, Math.random() < 0.5 ? "plea" : "fight").character;
